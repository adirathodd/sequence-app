...
export default function LobbyWaiting() {
  const { lobbySlots, hostId, roomCode, myPlayerId, numTeams, playersPerTeam, turnTimer, resetGame } = useGameStore()
  ...
  }, [rulesOpen, numTeams, playersPerTeam, turnTimer])

  function handleTeams(v: 2 | 3) { setLocalTeams(v); updateRules(v, localPPT, localTimer) }
  function handlePPT(v: 1 | 2 | 4) { setLocalPPT(v); updateRules(localTeams, v, localTimer) }
  function handleTimer(v: 15 | 30 | 60 | null) { setLocalTimer(v); updateRules(localTeams, localPPT, v) }
