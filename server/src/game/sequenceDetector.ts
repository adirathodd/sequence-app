import { Cell, ChipColor, Sequence } from '../types/game'
...
function cellMatchesColor(cell: Cell, color: ChipColor): boolean {
  return cell.chip === color || cell.card === 'FREE'
}

function coordsInSequence(seq: Sequence, r: number, c: number): boolean {
  ...
  for (const [dr, dc] of DIRECTIONS) {
    ...
    for (let i = -4; i <= 4; i++) {
      ...
      if (cellMatchesColor(board[r][c], color)) {
        ...
        if (run.length >= 5) tryAddSequence(run, existing, found, color, placedRow, placedCol)
        run.length = 0
      }
    }
    if (run.length >= 5) tryAddSequence(run, existing, found, color, placedRow, placedCol)
...
function tryAddSequence(
  ...
  for (let start = 0; start <= run.length - 5; start++) {
    const window = run.slice(start, start + 5) as [number, number][]

    // Must include the newly placed cell
    if (!window.some(([r, c]) => r === placedRow && c === placedCol)) continue
...
export function lockSequences(board: Cell[][], sequences: Sequence[]): Cell[][] {
